...
def update_VDBSCAN(popPos,popfit,it,trainX, testX, trainy, testy,best_x,best_f, best_acc,best_cols):
    ...
    db = VDBSCAN(kappa=0.005,metric=cosine).fit(np.array(popPos),eta=0.5)
    labels = db.labels_
    # print(labels)
...
def ARO_VDBSCAN(dim,max_it, npop,trainX, testX, trainy, testy , pop_pos_init, pop_fit_init,best_pop_init,best_fit_init,best_acc_init,best_cols_init,roound):
    ...
    for it in range(max_it-1):
        print('ARO-VDBSCAN,Itration : '+str(roound) +'-'+str(it)+'  Fitness: '+str(best_f)+'  Acc: '+str(best_acc)+
        ...
        for i in range(npop):
            ...
            if pop_fit[i] < best_f:
               ...
               best_acc=tempacc
               best_cols=tempcols
        pop_pos,pop_fit,best_x,best_f,best_acc,best_cols=update_VDBSCAN(pop_pos,pop_fit,it,trainX, testX, trainy, testy,best_x,best_f, best_acc,best_cols)
